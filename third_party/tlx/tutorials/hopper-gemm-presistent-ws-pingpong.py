...
@triton.jit
def wgmma_wait_kernel(N: tl.constexpr):
    tl.inline_asm_elementwise(
        "wgmma.wait_group.sync.aligned $1;", "=r, n", [N], dtype=tl.int32, is_pure=False, pack=1
    )
...
def matmul_kernel_tlx_ws_persistent(
    ...
    pingpong_mma_bar = tlx.alloc_barriers(num_barriers=NUM_MMA_GROUPS, arrive_count=1)
    pingpong_epi_bar = tlx.alloc_barriers(num_barriers=NUM_MMA_GROUPS, arrive_count=1)
    with tlx.async_tasks():
        ...
        with tlx.async_task(num_warps=4, replicate=2, registers=232):
            ...
            for tile_id in range(start_pid, num_tiles, NUM_SMS * 2):
                ...
                tlx.barrier_wait(bar=mma_bar, phase=phase_math)
                ...
                peer_mma = tlx.local_view(pingpong_mma_bar, 1 - cid)
                tlx.barrier_arrive(peer_mma) # release peer mma bar
                wgmma_wait_kernel(0)
                empty = tlx.local_view(mainloop_empty_bar, last_buf)
                tlx.barrier_arrive(empty)
...
for fp8_inputs in [False, True]:
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            x_vals=[8192], #[256 * i for i in range(2, 33)],  # Different possible values for `x_name`
